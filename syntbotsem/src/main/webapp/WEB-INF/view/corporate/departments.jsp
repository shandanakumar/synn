...
<title>SyntBots Corporate Admin - Department List</title>
...
<body class=" ">
	...
	<aside id="left-panel">
		...
		<nav>
			<ul>
				...
				<li class="active"><a href="departmentList" title="departments"><i
				...
				<li><a href="departmentQuota"><i
					...
					<li>
						...
						<ul>
							<li>
								<a href="bespokeBluePrint">Besopke</a>
							</li>
						</ul>		
					</li>
			</ul>
				...
				<div class="row">
					...
					<div class="pull-right" style="padding-right: 1%;">
						<a data-toggle="modal" onclick="getAdd()"
							...
							class="fa fa-circle-arrow-up fa-lg"></i> Create Department
						</a> <br> <br>
					</div>
					...
					<form name="department" id="department">
						...
						<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
							<div class="jarviswidget jarviswidget-color-darken" id="wid-id-1"
								...
								<a title="assignVdctoDpt" data-toggle="modal"id="${department.dpt_id}" name="${department.dpt_name}"onclick="assignVdctoDpt(this);"><i class="fa fa-sitemap"></i></a> 
								<a title="delete" href="#" id="${department.dpt_id}" name="${department.dpt_name}"onclick="deleteDepartment(this)" class="fa fa-trash-o"></i></a> <%--       </c:when>
                                                           <c:otherwise>
	...
	<div id="dialog_simple" title="Dialog Simple Title">
		<p>
			Are you sure you want to delete the department : <span id="dept">
			</span> ? .
		</p>
	</div>
	...
	<!-- /.modal -->
                   ...
                   <div  class="modal fade" id="noVdcModal"  tabindex="-1" role="dialog" >
                    ...
                    <div  class="modal fade" id="DeptProjModal"  tabindex="-1" role="dialog" >
						<div class="modal-dialog">	            
				            <div class="modal-content">
                                <div class="modal-header">
                                   <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
                                ...
                                <p>
                                Projects  are allocated for this "<span id="dept1"> </span>".Ask department Admin to release the following projects.
	...
	<script type="text/javascript">
		...
		function deleteDepartment(department) {
	        ...
	        //$('#dynspan').append('<span>'+'&nbsp'+':'+'</span>');
		 	$.getJSON("./getAvailabledDeptProj/"+deptId,function(response){
	       		if(!response.length){
	       			$('#dept').text(name);
	    			$('#deptId').val(deptId);
	       		...
	       		for (i = 0; i < response.length; i++) {
	       			...
	       			if(j<=8)
	       				{
	       			 $('#dynspan').append('<span>'+response[i].proj_name+'&nbsp;'+','+'&nbsp;'+'</span>');
	       				}
	       			else
	       				{
	       				j=1;
	       			 $('#dynspan').append('<span>'+response[i].proj_name+'&nbsp;'+','+'&nbsp;'+'</span>'+'<br>');	
	       				}
		       		j++;
		...
		$("#dept_name").change(function() {
			 ...
			 $("#messagename").html('<img src="img/discoverImage.GIF" align="absmiddle">&nbsp;Checking availability...');
			     ...
			     $.ajax({
		                    type:"post",
		                    url:"./checkDeptNameAvailability",
		                    data:"dept_name="+dept_name,
		                        success:function(data){
		                        if(data==0){
		                            $("#messagename").html(" <font color='green'> <b>Department available </b> </font> ");
		                        }
		                        else{

		                          

		                            $("#messagename").html("  <font color='red'>Department already Exist!  </font>  ");
