...
<%@ page import="com.syntel.isap.provisioning.bean.VmProvision"%>

<!DOCTYPE html>
<html lang="en-us">
	<head>
		<meta charset="utf-8">
		<title> SyntBots - Bespoke Service List</title>
	...
	<%
      ...
      List<VmProvision> bespokeList= null;
        if(request.getAttribute("list")!= null){
            bespokeList= (List<VmProvision>)request.getAttribute("list");   
        }else{           
            bespokeList = new ArrayList<VmProvision>();
        }
       
        VmProvision objCustomVM = null;
        String strBespokeVMsToFetchIPAddresses = "";
        for(int i=0;i < bespokeList.size(); i++){
            objCustomVM = (VmProvision) bespokeList.get(i);
           
            if((objCustomVM.getIp_addr() == null) || (objCustomVM.getIp_addr().length() == 0) || (objCustomVM.getIp_addr().equals("-"))){
                if(strBespokeVMsToFetchIPAddresses.length() > 0)
                {
                    strBespokeVMsToFetchIPAddresses += ",";
                }
                strBespokeVMsToFetchIPAddresses += objCustomVM.getVm_name();
            }
     ...
     <body class="menu-on-top">    
		...
		<!-- end collapse menu -->
         ...
         <span> <a style="border-radius: 2px;cursor: default !important;display: inline-block;font-weight: 700;height: 30px;line-height: 24px;min-width: 30px; padding: 2px;text-align: center;text-decoration: none !important;-moz-user-select: none;background-color: #F8F8F8;   
                          ...
                          margin: 10px 0px 0px;" href="./logout" title="Sign Out" data-action="userLogout" data-logout-msg="You can improve your security further after logging out by closing this opened browser"><i class="fa fa-sign-out"></i>
                 </a> 
          </span> 
         </div> --%>
				   ...
				   <li>
					...
					<li class="active">
						...
						<ul>
							...
							<li class="active">
								<a  href="bespokeUserServiceList" title="Bespoke Service List">Bespoke Service List </a>
							</li>
						</ul>
					</li>							
		...
		<div id="main" role="main">
			...
			<div id="ribbon">
				...
				<!-- breadcrumb -->
				<ol class="breadcrumb">
					<li>Home</li><li>Service List</li><li>Bespoke Service List</li>
				</ol>
			</div>
			<!-- END RIBBON -->
									    ...
									    <thead>								
												<tr>
												 <!--   <th><input type="checkbox" id="ckbCheckAll"></th>  -->
													...
													<th data-hide="phone,tablet">Components</th>
													<th data-hide="phone,tablet">Start</th>
													<th data-hide="phone,tablet">End</th>
													<th data-hide="phone,tablet">Provider Type</th>
											...
											<tr>
													      <%--  <td align="center"><input type="checkbox" id="${customList.vm_name}" value="${customList.vm_custom_id}" class="checkBoxClass"></td> --%>
															...
															<c:set var="publicIpAddr" value="${customList.public_ip_addr}"/>
															<c:choose>
																    <c:when test="${ publicIpAddr =='-'}">
																		<td> ${customList.public_ip_addr}</td>
																	</c:when>
																	<c:otherwise>
																   
																    <td><a href = "http://${customList.public_ip_addr}:8080">http://${customList.public_ip_addr}:8080</a></td>															     
																   </c:otherwise>
																</c:choose>
															...
															<td>
															<%-- <c:forEach items="${customList.components}" var="component">
															
															${component} &nbsp;&nbsp;${customList.bspoke_mst_vm_name}
															
															</c:forEach> --%>
															${customList.bspoke_mst_vm_name}
														  </td>
															<td>${customList.startDate}   </td>
															<td>${customList.endDate}  </td>
															<td>${customList.vdc_name}  </td>
														  <%--  <c:set var="paramName" value="${customList.status}"/>
															<td>
																 <c:choose>
																    <c:when test="${ paramName=='Requested'}">
																	 Provisioning &nbsp; <img src="img/discoverImage.GIF">
																   </c:when>
																   ...
																   <c:otherwise>
																   ${customList.status}  &nbsp; <img src="img/discoverImage.GIF">
													...
													<c:set var="paramName" value="${customList.status}"/>
														...
														<a title="Terminate VM" id="${customList.vm_id}" name="${customList.vm_name}" onclick="terminateInstance(this)" href="#"><i style="color:red" class="fa fa-trash-o" id="delete_icon"></i></a>															
														&nbsp;&nbsp;
														<div class="btn-group">		
															...
															<ul class="dropdown-menu">	
																...
																<c:choose>	
																	<c:when test="${(cloudParam=='OpenStack-CHN')}">	
																		 <c:choose>													 																																	
																			...
																			<c:when test="${(ipAdd=='-') and (paramName=='ACTIVE') }">				
												                               <li>												
															                   		<a id="${customList.vm_id}" name="${customList.vm_name}" href="#" onclick="addFloatingIP(this);">Allocate Floating IP</a>												
														                       </li>													
														  	                   <li>												
																	     	...
																	     	<c:when test="${(ipAdd=='0.0.0.0') and (paramName=='ACTIVE') }">				
																	  		...
																	  		<c:otherwise>
																		        <li>
																		   	     ...
																		   	     <li>
																					<a id="${customList.vm_id}" name="${customList.vm_name}" href="#" onclick="removeFloatingIP(this);">Deallocate Floating IP</a>
																				</li>
																	 		</c:otherwise>
																   		 </c:choose>	
																		 <li>
																	...
																	<c:when test="${(cloudParam=='MS-AZURE-STG')}">	
																	       <li>
	   ...
	   // DO NOT REMOVE : GLOBAL FUNCTIONS!		
		$(document).ready(function() {	
		   ...
		   submitButt = $("#anc_id");
			...
			setTimeout(function(){
				launchAzureBespokeInstanceDBUpdate();
		   		},1000);
		});
		...
		<script>
	function launchAzureBespokeInstanceDBUpdate(){
		var varBespokeVMsToFetchIPAddresses = '<%= strBespokeVMsToFetchIPAddresses %>';
		
		//alert(varBespokeVMsToFetchIPAddresses);

    	 $.ajax({     
    		type: "POST",     
    		data: "strJsonObj="+varBespokeVMsToFetchIPAddresses,
    		url: "./azurebespokelaunchdbupdate",		
