package com.syntel.isap.provisioning.bean;

import java.io.Serializable;
import java.util.List;

public class EnvironmentVM implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	private int vm_master_id;
	private List<Vdc> vdcList;

	private String vm_name;

	private String hostname;

	private String mem;

	private String cpu;

	private String hdd;

	private String network;

	private String os_type;

	private String os;

	private String vdc_provider;

	private String req_type;

	private String virt_type;

	private String status;

	private String created_at;

	private String modified_at;

	private String deleted_at;

	private String created_by;

	private String modified_by;

	private String deleted_by;

	private int firewall_master_id;
	
	private Vdc vdc;
	
	public List<Vdc> getVdcList() {
		return vdcList;
	}

	public void setVdcList(List<Vdc> vdcList) {
		this.vdcList = vdcList;
	}

	public Vdc getVdc() {
		return vdc;
	}

	public void setVdc(Vdc vdc) {
		this.vdc = vdc;
	}

	private List<BespokePackages> pkgList;
	public List<BespokePackages> getPkgList() {
		return pkgList;
	}

	public void setPkgList(List<BespokePackages> envVMPackageList) {
		this.pkgList = envVMPackageList;
	}

	public int getVm_master_id() {
		return vm_master_id;
	}

	public void setVm_master_id(int vm_master_id) {
		this.vm_master_id = vm_master_id;
	}

	public String getVm_name() {
		return vm_name;
	}

	public void setVm_name(String vm_name) {
		this.vm_name = vm_name;
	}

	public String getHostname() {
		return hostname;
	}

	public void setHostname(String hostname) {
		this.hostname = hostname;
	}

	public String getMem() {
		return mem;
	}

	public void setMem(String mem) {
		this.mem = mem;
	}

	public String getCpu() {
		return cpu;
	}

	public void setCpu(String cpu) {
		this.cpu = cpu;
	}

	public String getHdd() {
		return hdd;
	}

	public void setHdd(String hdd) {
		this.hdd = hdd;
	}

	public String getNetwork() {
		return network;
	}

	public void setNetwork(String network) {
		this.network = network;
	}

	public String getOs_type() {
		return os_type;
	}

	public void setOs_type(String os_type) {
		this.os_type = os_type;
	}

	public String getOs() {
		return os;
	}

	public void setOs(String os) {
		this.os = os;
	}

	public String getVdc_provider() {
		return vdc_provider;
	}

	public void setVdc_provider(String vdc_provider) {
		this.vdc_provider = vdc_provider;
	}

	public String getReq_type() {
		return req_type;
	}

	public void setReq_type(String req_type) {
		this.req_type = req_type;
	}

	public String getVirt_type() {
		return virt_type;
	}

	public void setVirt_type(String virt_type) {
		this.virt_type = virt_type;
	}

	public String getStatus() {
		return status;
	}

	public void setStatus(String status) {
		this.status = status;
	}

	public String getCreated_at() {
		return created_at;
	}

	public void setCreated_at(String created_at) {
		this.created_at = created_at;
	}

	public String getModified_at() {
		return modified_at;
	}

	public void setModified_at(String modified_at) {
		this.modified_at = modified_at;
	}

	public String getDeleted_at() {
		return deleted_at;
	}

	public void setDeleted_at(String deleted_at) {
		this.deleted_at = deleted_at;
	}

	public String getCreated_by() {
		return created_by;
	}

	public void setCreated_by(String created_by) {
		this.created_by = created_by;
	}

	public String getModified_by() {
		return modified_by;
	}

	public void setModified_by(String modified_by) {
		this.modified_by = modified_by;
	}

	public String getDeleted_by() {
		return deleted_by;
	}

	public void setDeleted_by(String deleted_by) {
		this.deleted_by = deleted_by;
	}

	public int getFirewall_master_id() {
		return firewall_master_id;
	}

	public void setFirewall_master_id(int firewall_master_id) {
		this.firewall_master_id = firewall_master_id;
	}
}
